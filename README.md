...
4. If you need a starter file, download the [CSV template](templates/psx_eod_template.csv) and replace the sample rows.

## Data format
- **Columns (required order)**: `Symbol, Date, Open, High, Low, Close, Volume`.
- **Date**: `YYYY-MM-DD`.
- **Symbol naming**: include one row per trading day for each symbol you care about **and** daily rows for `KSE100` so relative strength can be calculated.
- **One row per day**: end-of-day data only; no intraday bars.
